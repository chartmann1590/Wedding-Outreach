...
from services.sheets import parse_public_url, to_csv_url, fetch_csv_data, process_guest_data
...
def review():
    ...
    for guest in guests:
        ...
        if guest.facebook_profile:
            ...
            msg_link = f"https://www.facebook.com/messages/t/{clean_name}" if clean_name else "https://www.facebook.com/messages"
        ...
        if ollama_available:
            message = draft_message(first_name, setting.ollama_base, setting.ollama_model, wedding_details, timeout=5)
        else:
...
def get_ollama_models():
    ...
    if not ollama_base:
        return jsonify({'success': False, 'message': 'No Ollama base URL provided', 'models': []})
...
def upload_csv():
    ...
    try:
        ...
        if not guests_data:
            return jsonify({'success': False, 'message': 'No valid guest data found in CSV'})
        ...
        setting.csv_address_field = getattr(process_guest_data, '_address_field', 'Address')
        setting.csv_notes_field = getattr(process_guest_data, '_notes_field', 'Notes')
        setting.csv_facebook_field = getattr(process_guest_data, '_facebook_field', 'facebook_profile')
        setting.updated_at = datetime.utcnow()
