...
def smart_detect_status(address, note):
    ...
    facebook_patterns = [
        # Direct statements
        'no facebook', 'not on facebook', 'no fb', 'not on fb', 'not facebook', 'no face book',
        'facebook not found', 'fb not found', 'cant find facebook', "can't find facebook",
        'cannot find.*?facebook', 'cannot find.*?fb', 'could not find.*?facebook', 'could not find.*?fb',
        'not found on facebook', 'not found on fb', 'no facebook profile', 'no fb profile',
        'no facebook account', 'no fb account', 'facebook account not found', 'fb account not found',
        'doesnt have facebook', "doesn't have facebook", 'does not have facebook',
        'doesnt use facebook', "doesn't use facebook", 'does not use facebook',
        'no social media', 'not on social media', 'no social', 'not social',
        'facebook inactive', 'fb inactive', 'inactive facebook', 'inactive fb',
        'facebook deactivated', 'fb deactivated', 'deactivated facebook', 'deactivated fb',
        # Match patterns
        'facebook match', 'fb match', 'facebook found', 'fb found',
        # Variations with dates/timestamps
        'facebook.*?20[0-9][0-9]', 'fb.*?20[0-9][0-9]',
        # Plus one variations
        'plus one.*?not required', 'plus.*?one.*?not.*?required', r'\+1.*?not.*?required',
        # General finding patterns
        'find.*?facebook', 'find.*?fb', 'locate.*?facebook', 'locate.*?fb',
        'search.*?facebook', 'search.*?fb'
    ]
    
    # Address request patterns (indicating address was requested)
    address_patterns = [
        # Direct statements  
        'address requested', 'asked for address', 'request address', 'requested address',
        'address asked', 'address needed', 'need address', 'needs address',
        'invitation sent', 'invite sent', 'invited', 'invitation mailed',
        'reached out', 'contacted', 'messaged', 'texted', 'called', 'emailed',
        'waiting for address', 'pending address', 'address pending',
        'follow up', 'following up', 'follow-up', 'will follow up',
        'reminded', 'reminder sent', 'second request', '2nd request',
        # Status indicators
        'in progress', 'working on', 'tracking', 'pursuing'
    ]
...
def messenger_search_link(guest_name, message=None):
    ...
    if guest_name:
        ...
        clean_name = guest_name.lower().replace(' ', '').replace('(', '').replace(')', '').replace('.', '').replace(',', '')
...
def get_models():
    ...
    if not ollama_base:
        return jsonify({'success': False, 'models': [], 'message': 'No Ollama base URL provided'})
...
def test_refresh():
    ...
    try:
        ...
        return jsonify({'error': f'Test error: {str(e)}', 'debug': 'Exception in test endpoint'})
...
def detect_csv_fields(headers):
    ...
    name_candidates = ['name', 'full name', 'fullname', 'guest name', 'guest', 'person', 'contact']
    name_field = None
    ...
    address_candidates = ['address', 'mailing address', 'street address', 'addr', 'location', 'home address']
    address_field = None
    ...
    notes_candidates = ['notes', 'note', 'comments', 'comment', 'description', 'details', 'info', 'remarks']
    notes_field = None
    ...
    facebook_candidates = ['facebook', 'fb', 'facebook profile', 'fb profile', 'social media', 'facebook_profile']
    facebook_field = None
            ...
            if row_num is not None and 0 <= row_num - 1 < len(rows):
                ...
                if field_mappings['notes'] and 'status' in updates:
                    ...
                    for pattern in status_markers:
                        current_notes = re.sub(pattern, '', current_notes, flags=re.IGNORECASE).strip()
                    ...
                    if status_note:
                        if current_notes:
                            row[field_mappings['notes']] = f"{current_notes} | {status_note}"
                        else:
...
def upload_csv():
    ...
    try:
        ...
        for row_num, row in enumerate(csv_reader, 1):
            name = row.get(field_mappings['name'], '').strip() if field_mappings['name'] else ''
            if not name or name.lower() in ['nan', 'none', '']:
                print(f"DEBUG: Skipping row {row_num} - empty name: '{name}'")
                continue
                
            address = row.get(field_mappings['address'], '').strip() if field_mappings['address'] else ''
            note = row.get(field_mappings['notes'], '').strip() if field_mappings['notes'] else ''
            facebook_profile = row.get(field_mappings['facebook'], '').strip() if field_mappings['facebook'] else ''
...
def refresh_sheet():
    ...
    return jsonify({'error': 'Please use the CSV file upload feature instead of Google Sheets URL'}), 400
...
def review():
    ...
    for guest in guests:
        ...
        if setting and setting.ollama_base and setting.ollama_model:
            try:
                message = draft_message(guest.name, setting.ollama_base, setting.ollama_model)
            except Exception as e:
