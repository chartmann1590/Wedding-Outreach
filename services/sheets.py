...
def process_guest_data(df: pd.DataFrame) -> list:
    ...
    if not name_col:
        raise ValueError("CSV must contain a column with 'name' or 'guest' in the header")
    ...
    for _, row in df.iterrows():
        ...
        address = str(row.get(address_col, '')).strip() if address_col and pd.notna(row.get(address_col)) else ''
        
        guest = {
            'name': name,
            'address': address,
            'note': notes,
            'facebook_profile': str(row.get(facebook_col, '')).strip() if facebook_col and pd.notna(row.get(facebook_col)) else '',
        }
